...
public:
	 ...
	 * This object will be changed in the course of applying this StateAlgorithm.
	 */
	PolicyInitialisation(const PolicyPtrT& p, const ActionGeneratorConstPtrT& _actionGenerator): policy(p), actionGenerator(_actionGenerator){
		assert(actionGenerator.get());
		assert(policy.get());
	}
...
std::shared_ptr<Policy<State,Action> > policyIteration(
	...
	PolicyIterationUpdatePtrT policyIterationUpdate(new PolicyIterationUpdateT(utility,transition,policy,actionGen));
	bool unchanged=true;

	ValueIterationUpdatePtrT valueIterationUpdate(new ValueIterationUpdateT(utility,reward,transition,NULL,policy,discount,0));

	PolicyInitialisationPtrT policyInit(new PolicyInitialisationT(policy,actionGen));
