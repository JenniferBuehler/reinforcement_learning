...
public:
	...
	MaxUtilityActionAlgorithm(const UtilityT& _u, const TransitionT& _t, const StateT& _s): u(_u),t(_t),s(_s),maxVal(0),maxAction(ActionT()){}
	...
	virtual bool apply(const ActionT& a){
		...
		if (!equalFloats(probCnt,1.0f,(float)ZERO_EPSILON)){
