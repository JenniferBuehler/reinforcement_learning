...
public:
    ...
    explicit LearningRate(float initialValue): learnRate(initialValue)
    {
...
{
public:
    // use intitial learning rate of 0.99 because the learn rate is decaying
    explicit DecayLearningRate(float _decayRate = 1e-01): LearningRate(0.99), decayRate(_decayRate) {}
    virtual ~DecayLearningRate() {}
    virtual float get(unsigned int freq) const
    {
        double decayFact = this->learnRate * decayRate;
        float adaptedLearnRate = static_cast<double>(learnRate) / (1.0 + decayFact * freq);
