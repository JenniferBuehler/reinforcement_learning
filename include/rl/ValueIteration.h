...
public:
	...
	virtual bool apply(const StateT& s){
		...
		if ((utChange > delta) && !equalFloats(utChange,delta,(float)ZERO_EPSILON))
			delta=utChange;
		return true;
	}
	float getDelta(){
...
*/
	...
	float delta=0;
	float discountRatio=(float)(1.0-discount)/(float)discount;
	float minDelta=maxErr*discountRatio;
	PRINTMSG("Starting value iteration with discount="<<discount<<", discountRatio="<<discountRatio<<", maxErr="<<maxErr<<", minDelta="<<minDelta);
	unsigned int cnt=0;
	ValueIterationUpdate<State,Action> valueIterationUpdate(utility,reward,transition,actionGen,nullPolicy,discount,delta);
	do{
	...
	}while((delta > minDelta) || ((delta > minDelta) && (!equalFloats(delta,minDelta,(float)ZERO_EPSILON))));
